...
int optoEdgeGet(int dev, uint8_t ch, uint8_t *val)
{
	...
	uint8_t buf[4];
	if (OK != i2cMem8Read(dev, I2C_MEM_OPTO_COUNT_ENABLE, buf, 2))
	{
		return ERROR ;
	}
...
int optoEdgeSet(int dev, uint8_t ch, uint8_t val)
{
	...
	if (OK != i2cMem8Write(dev, I2C_MEM_OPTO_COUNT_ENABLE, buf, 2))
	{
		return ERROR ;
	}
	return OK ;
...
const CliCmdType CMD_OPTO_READ =
{
	...
	"  Usage:           "PROGRAM_NAME" optrd <channel>\n"
	"  Usage:           "PROGRAM_NAME" optrd\n",
	"  Example:         "PROGRAM_NAME" optrd 2; Read Status of Optocoupled input ch #2\n"
};
...
const CliCmdType CMD_OPTO_EDGE_READ =
{
	...
	1,
	&doOptoEdgeRead,
	"  optedgerd        Read optocoupled counting edges 0 - none; 1 - rising\n",
	"  Usage:           "PROGRAM_NAME" optedgerd <channel>\n",
	"  Example:         "PROGRAM_NAME" optedgerd 2; Read counting edges of optocoupled channel #2\n"
};
...
const CliCmdType CMD_OPTO_EDGE_WRITE =
{
	...
	"  Usage:           "PROGRAM_NAME" optedgewr <channel> <edges> \n",
	"  Example:         "PROGRAM_NAME" optedgewr 2 1; Set Optocoupled channel #2 to count rising edges\n"
};
int doOptoEdgeWrite(int argc, char *argv[])
{
	...
	else if (strcasecmp(argv[3], "up") == 0
		|| strcasecmp(argv[3], "rising") == 0)
	{
		state = 1;
	}
...
const CliCmdType CMD_OPTO_CNT_READ =
{
	...
	"  Usage:           "PROGRAM_NAME" optcntrd <channel>\n",
	"  Example:         "PROGRAM_NAME" optcntrd 2; Read contor of opto input #2\n"
};
...
const CliCmdType CMD_OPTO_CNT_RESET =
{
	...
	"  Usage:           "PROGRAM_NAME" optcntrst <channel>\n",
	"  Example:         "PROGRAM_NAME" optcntrst 2; Reset contor of opto input #2\n"
};
